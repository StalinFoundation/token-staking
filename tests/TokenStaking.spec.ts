...
describe('TokenStaking', () => {
    ...
    beforeEach(async () => {
        ...
        tokenStaking = blockchain.openContract(
            TokenStaking.createFromConfig(
                {
                    owner: admin.address,
                    collection_content: 'https://raw.githubusercontent.com/StalinFoundation/Images/main/metadata.json',
                    nft_content: 'https://raw.githubusercontent.com/StalinFoundation/Images/main/',
                    numerator: 11,
                    denominator: 1000,
                    royalty_address: admin.address,
                    nft_item_code: nft_code,
                    staking_conf: {
                        max_percent: Math.floor(15000000),
                        min_percent: Math.floor(1000000),
                        reward_supply: toNano(1000000),
        ...
        jettonMaster = blockchain.openContract(
            JettonMinter.createFromConfig(
                { adminAddress: admin.address, content: '', jettonWalletCode: jetton_code },
                minter_code
            )
        );
        ...
        jettonStaking = blockchain.openContract(
            JettonWallet.createFromAddress(await jettonMaster.getWalletAddress(tokenStaking.address))
        );
        jettonUser = blockchain.openContract(
            JettonWallet.createFromAddress(await jettonMaster.getWalletAddress(user.address))
        );
        await jettonMaster.sendMint(admin.getSender(), {
            toAddress: user.address,
            jettonAmount: toNano(1500000),
            queryId: 123,
            value: toNano(1),
            amount: toNano('0.5'),
        });
    ...
    it('should set default content', async () => {
        expect(await tokenStaking.getNftContent(0, beginCell().endCell())).toEqual(
            'https://raw.githubusercontent.com/StalinFoundation/Images/main/default.json'
        );
        await tokenStaking.sendChangeUseDefault(admin.getSender(), toNano(1), false);
        expect(await tokenStaking.getNftContent(0, beginCell().endCell())).toEqual(
            'https://raw.githubusercontent.com/StalinFoundation/Images/main/0.json'
        );
    ...
    it('should receive donate', async () => {
        await tokenStaking.sendSetJettonWallet(admin.getSender(), toNano(1), jettonStaking.address);
        expect(await tokenStaking.getJettonWalletAddress()).toEqualAddress(jettonStaking.address)
        let res = await jettonUser.sendTransfer(user.getSender(), {
            value: toNano(2),
            toAddress: tokenStaking.address,
            queryId: 123,
            fwdAmount: toNano(1),
            jettonAmount: toNano(1000),
            fwdPayload: 0x47bbe425,
        });
        expect(res.transactions).toHaveTransaction({
            from: user.address,
    ...
    it('should receive tokens and send nfts back', async () => {
        ...
        await tokenStaking.sendSetJettonWallet(admin.getSender(), toNano(1), jettonStaking.address);
        await jettonUser.sendTransfer(user.getSender(), {
            value: toNano(2),
            toAddress: tokenStaking.address,
            queryId: 123,
            fwdAmount: toNano(1),
        ...
        expect(res.transactions).toHaveTransaction({
            from: tokenStaking.address,
            to: Address.parse("UQBR7K8tQ7qoCAc07D4sQB3aREDWXtyGzfC73qMFHz1gfl3l"),
            value: toNano("0.1"),
            body: comment("Fee 💎")
        })

        let nft = blockchain.openContract(
            NftItem.createFromConfig({ index: 0, master: tokenStaking.address }, nft_code)
        );
    ...
    it('should receive tokens and send nfts back', async () => {
        ...
        let nft_2 = blockchain.openContract(
            NftItem.createFromConfig({ index: 1, master: tokenStaking.address }, nft_code)
        );
    ...
    it('should receive tokens and send nfts back', async () => {
        ...
        expect((await nft.getNftContent()).locked_amount).toEqual(toNano(1000 + 150 * 3));
        expect(await nft.getOwner()).toEqualAddress(user.address);
        expect((await nft.getNftContent()).lockup_time).toEqual(blockchain.now + 3600 * 24 * 30 * 3);
        expect((await tokenStaking.getStakingData()).current_supply).toEqual(toNano(1000000 - 150 * 3));
        expect((await tokenStaking.getStakingData()).current_balance).toEqual(toNano(1000000 + 1000));
    });
    it('should change config', async () => {
        let balance_before = (await blockchain.getContract(tokenStaking.address)).balance;
        let expected_config = {max_percent: Math.floor(1000),
            min_percent: Math.floor(100),
            reward_supply: toNano(20000),
            lockup_period: 30,
            min_cycles: 2,
            max_cycles: 10}
        let c = await tokenStaking.sendChangeConfig(admin.getSender(), toNano(1), expected_config);
        printTransactionFees(c.transactions);
        let res = await tokenStaking.getStakingData();
        expect(res.staking_conf).toMatchObject(expected_config)
    });
});
