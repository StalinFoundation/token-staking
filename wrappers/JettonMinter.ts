...
export class JettonMinter implements Contract {
    ...
    async sendMintAccess(
        ...
        await provider.internal(via, {
            value: opts.value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(5, 32).storeUint(opts.queryId, 64).storeAddress(opts.mintAccess).endCell(),
        });
    }

    async sendDeleteMintAccess(
        provider: ContractProvider,
        via: Sender,
        opts: {
            mintAccess: Address;
        ...
        await provider.internal(via, {
            value: opts.value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(6, 32).storeUint(opts.queryId, 64).storeAddress(opts.mintAccess).endCell(),
        });
    }
